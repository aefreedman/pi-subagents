...
function getNestedDelegationBlock(toolName: "subagent" | "subagent_list") {
	...
	const actor = context.parentAgent || context.rootAgent || "delegated worker";
	return {
		content: [
			{
				type: "text" as const,
				text: [
					`Nested ${toolName} usage is disabled for delegated workers (depth ${context.depth}, current worker: ${actor}).`,
					"Return a parent handoff instead of delegating further.",
					"Include sections named: Missing Context, Why It Matters, Recommended Follow-up, Suggested Agent, Suggested Inputs.",
				].join("\n"),
			},
		],
		details: {
...
function formatAgentOriginLabel(agent: { source: "user" | "project" | "unknown"; sourceDetail?: string; packageName?: string }): string {
	if (agent.source === "unknown") return "unknown";
	if (!agent.sourceDetail) return agent.source;
...
function didResultFail(result: SingleResult): boolean {
	return (
		result.exitCode !== 0 ||
		result.stopReason === "error" ||
		result.stopReason === "aborted" ||
		result.contractFailed === true
	);
}

function formatParallelResultContent(results: SingleResult[]): string {
	const successCount = results.filter((r) => !didResultFail(r)).length;
	const sections = results.map((r) => {
		const status = didResultFail(r) ? "failed" : "completed";
		const output = getFinalOutput(r.messages);
		const body = didResultFail(r) ? r.errorMessage || r.stderr || output || "(no output)" : output || "(no output)";
...
function getSpinnerFrame(theme: any): string {
	const frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
	const index = Math.floor(Date.now() / 120) % frames.length;
	return theme.fg("warning", frames[index]);
}

function buildSingleResultHeader(result: SingleResult, theme: any, icon: string): string {
	let header = `${icon} ${theme.fg("toolTitle", theme.bold(result.agent))}${theme.fg("muted", ` (${formatAgentOriginLabel({ source: result.agentSource, sourceDetail: result.agentSourceDetail, packageName: result.agentPackageName })})`)}`;
	if (didResultFail(result) && result.stopReason) header += ` ${theme.fg("error", `[${result.stopReason}]`)}`;
	if (result.contractFailed) header += ` ${theme.fg("warning", `[contract]`)}`;
	return header;
}

function shortenPreview(text: string, maxLength = 120): string {
...
function renderRunningAggregateText(
	modeLabel: string,
	status: string,
	results: SingleResult[],
	theme: any,
...
function appendToolCallItems(container: Container, displayItems: DisplayItem[], theme: any): void {
...
async function runSingleAgent(
	...
	if (!agent) {
		const available = agents.map((a) => `"${a.name}" (${formatAgentSourceTag(a)})`).join(", ") || "none";
		return {
			agent: agentName,
			agentSource: "unknown",
			task,
	...
	const currentResult: SingleResult = {
		...
		agentSourceDetail: agent.sourceDetail,
	...
	const SPINNER_REFRESH_MS = 100;
	let lastUpdateAt = 0;
	let pendingUpdateTimer: NodeJS.Timeout | null = null;
	let spinnerRefreshTimer: NodeJS.Timeout | null = null;
	const emitUpdate = (force = false) => {
		...
		if (force || now - lastUpdateAt >= STREAM_UPDATE_THROTTLE_MS) {
		...
		if (!pendingUpdateTimer) {
			pendingUpdateTimer = setTimeout(emit, Math.max(0, STREAM_UPDATE_THROTTLE_MS - (now - lastUpdateAt)));
		}
	};
	const startSpinnerRefresh = () => {
		if (!onUpdate || spinnerRefreshTimer) return;
		spinnerRefreshTimer = setInterval(() => {
			if (currentResult.exitCode === -1) emitUpdate(true);
		}, SPINNER_REFRESH_MS);
	};
	const stopSpinnerRefresh = () => {
		if (spinnerRefreshTimer) {
			clearInterval(spinnerRefreshTimer);
			spinnerRefreshTimer = null;
		}
		if (pendingUpdateTimer) {
			clearTimeout(pendingUpdateTimer);
			pendingUpdateTimer = null;
	...
	try {
		...
		const delegationPacket = buildDelegationPacket({
			agent,
			task,
			cwd: cwd ?? defaultCwd,
			defaultCwd,
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			const proc = spawn(invocation.command, invocation.args, {
				cwd: cwd ?? defaultCwd,
				env: buildDelegatedChildEnv(agent.name),
				shell: false,
		...
		const validation = validateOutputContract(agent, getFinalOutput(currentResult.messages));
		if (!validation.ok) {
			currentResult.contractFailed = true;
			currentResult.contractIssues = validation.issues;
			currentResult.errorMessage = `Output contract failed: ${validation.issues.join("; ")}`;
		}
...
const AgentScopeSchema = StringEnum(["user", "project", "both"] as const, {
	description: 'Which agent sources to use. Default: "user". Use "both" to include project-scoped local/config/package agents.',
	default: "user",
});
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.chain && params.chain.length > 0) {
				...
				for (let i = 0; i < params.chain.length; i++) {
					...
					const isError = didResultFail(result);
					if (isError) {
						const errorMsg =
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				return {
					content: [
						{
							type: "text",
							text: formatParallelResultContent(results),
						},
					],
		...
		renderResult(result, { expanded }, theme, _context) {
			...
			if (details.mode === "single" && details.results.length === 1) {
				...
				const isError = didResultFail(r);
				const icon = isRunning ? getSpinnerFrame(theme) : isError ? theme.fg("error", "✗") : theme.fg("success", "✓");
				const displayItems = getDisplayItems(r.messages);
				const finalOutput = getFinalOutput(r.messages);

				if (isRunning) {
					return new Text(renderRunningSingleText(r, theme, icon), 0, 0);
				}

				if (expanded) {
					const container = new Container();
					container.addChild(new Text(buildSingleResultHeader(r, theme, icon), 0, 0));
			...
			if (details.mode === "chain") {
				...
				const successCount = details.results.filter((r) => r.exitCode !== -1 && !didResultFail(r)).length;
				const failCount = details.results.filter((r) => r.exitCode !== -1 && didResultFail(r)).length;
				const isRunning = running > 0;
				const icon = isRunning
					? getSpinnerFrame(theme)
					: failCount > 0
						? theme.fg("error", "✗")
						: theme.fg("success", "✓");

				if (isRunning) {
					const status = `${successCount + failCount}/${details.results.length} done, ${running} running`;
					return new Text(renderRunningAggregateText("chain", status, details.results, theme, icon), 0, 0);
				}
			...
			if (details.mode === "parallel") {
				...
				if (isRunning) {
					return new Text(renderRunningAggregateText("parallel", status, details.results, theme, icon), 0, 0);
				}
